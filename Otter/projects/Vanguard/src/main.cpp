...
std::string windowTitle = "Vanguard";
...
int main() {
	...
	VertexArrayObject::Sptr vao4 = ObjLoader::LoadFromFile("vangaurd1.obj");
	...
	while (!glfwWindowShouldClose(window)) {
		...
		if (glfwGetKey(window, GLFW_KEY_SPACE))
		{
			if (!isButtonPressed)
			{
				...
				if (camera->GetOrthoEnabled() == false)
				{
					//sets camera to orthographic
					camera->SetOrthoEnabled(true);
					camera->SetOrthoVerticalScale(5);
				}
				else if (camera->GetOrthoEnabled() == true)
				{
		...
		//static_cast<float>(thisFrame)
		// TODO: Week 5 - toggle code

		// Rotate our models around the z axis
		if (isRotating) {
			transform  = glm::rotate(glm::mat4(1.0f), glm::radians(8.0f * dt), glm::vec3(0, 1, 0));
		}
		transform2 = glm::translate(glm::mat4(1.0f), glm::vec3(1.5f, 0.0f, 0.0f));
		transform3 = glm::translate(glm::mat4(1.0f), glm::vec3(-1.5f, 0.0f, 0.0f));
		...
		//rotates then translates
		//shader->SetUniformMatrix("u_ModelViewProjection", camera->GetViewProjection() * transform2 * transform);
		//vao4->Draw();
